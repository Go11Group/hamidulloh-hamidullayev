...
func (ush *Handler) User_GetAll(gn *gin.Context) {
	users := []model.User{}
	users, err := ush.User.GetAllUser()
	if err != nil {
	...
	// Agar foydalanuvchilar bo'lsa, 200 status kodi bilan barcha foydalanuvchilarni JSON ko'rinishida qaytarish kerak
	gn.JSON(http.StatusOK, users)
}